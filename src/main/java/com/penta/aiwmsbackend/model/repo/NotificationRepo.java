...
public interface NotificationRepo extends JpaRepository<Notification, Integer> {
    ...
    @Query(value = "SELECT DISTINCT(n.id) , n.* FROM notifications n LEFT JOIN boards b ON n.board_id = b.id LEFT JOIN boards_has_users bs ON n.board_id = bs.board_id WHERE ( n.sent_user_id != ?1 AND bs.user_id = ?1 AND bs.joined_status = true AND  b.delete_status = false AND n.is_invitiation = false ) OR ( n.sent_user_id != ?1  AND b.user_id = ?1 AND b.delete_status = false ) OR ( n.sent_user_id != ?1 AND n.is_invitiation = true AND bs.user_id = ?1 AND n.target_user_id = ?1  AND b.user_id != ?1 AND b.delete_status = false )  order by n.created_date desc ", nativeQuery = true)
